...
NODE_ORANGE = rgb('#ff8a3d')
PYTHON_GREEN = rgb('#43a047')
API_PURPLE = rgb('#7e57c2')
UI_BLUE = rgb('#1e88e5')
...
def add_code_block(slide, left, top, width, height, code, *, font_size=12, color=CODE_TEXT,
                   highlight_lines=None, highlight_color=ACCENT):
    shape = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, left, top, width, height)
    ...
    for index, line in enumerate(code.splitlines()):
        ...
        style_run(
            ...
            name='Courier New',
            size=font_size,
            color=highlight_color if highlight_lines and index in highlight_lines else color,
            bold=bool(highlight_lines and index in highlight_lines),
        )
    return shape
...
steps = ['Search', 'View', 'Confirm', 'Email', 'Audit', 'Candidate', 'Indicator']
for idx, step in enumerate(steps):
    x = Inches(0.85) + Inches(1.72) * idx
    step_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, x, Inches(4.55), Inches(1.25), Inches(0.55))
    apply_fill(step_box, CALLOUT_BG)
    apply_line(step_box, ACCENT, 1.25)
    add_centered_lines(step_box, [step], [TITLE], [12], [True])
    if idx < len(steps) - 1:
        add_arrow(slide, x + Inches(1.28), Inches(4.67), Inches(0.28), Inches(0.20), 'right', ACCENT)
add_callout(slide, Inches(1.05), Inches(5.45), Inches(11.1), Inches(0.70),
...
add_bullets(slide, Inches(1.05), Inches(4.15), Inches(10.9), Inches(2.0), [
...
add_bullets(slide, Inches(0.75), Inches(1.35), Inches(11.6), Inches(1.9), [
    ...
    'Used by Claude Desktop, GitHub Copilot, Cursor, and custom agents like this one',
], font_size=15)
add_textbox(slide, Inches(1.45), Inches(3.40), Inches(3.9), Inches(0.25), 'Old way',
            font_size=15, color=RED, bold=True, align=PP_ALIGN.CENTER, margin=0.0)
for idx, label in enumerate(['AI App A', 'AI App B', 'AI App C']):
    box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(0.95), Inches(3.80 + idx * 0.50), Inches(1.35), Inches(0.35))
    apply_fill(box, CALLOUT_BG)
    apply_line(box, RED, 1.1)
    add_centered_lines(box, [label], [TITLE], [11], [False])
for idx, label in enumerate(['Tool X', 'Tool Y', 'Tool Z']):
    box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(3.85), Inches(3.80 + idx * 0.50), Inches(1.35), Inches(0.35))
    apply_fill(box, CALLOUT_BG)
    apply_line(box, RED, 1.1)
    add_centered_lines(box, [label], [TITLE], [11], [False])
for start_y in [Inches(3.98), Inches(4.48), Inches(4.98)]:
    for end_y in [Inches(3.98), Inches(4.48), Inches(4.98)]:
        add_connector_arrow(slide, Inches(2.30), start_y, Inches(3.85), end_y, color=RED, width=1.0)
add_textbox(slide, Inches(7.80), Inches(3.40), Inches(3.0), Inches(0.25), 'MCP way',
            font_size=15, color=ACCENT, bold=True, align=PP_ALIGN.CENTER, margin=0.0)
hub = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.HEXAGON, Inches(8.00), Inches(4.15), Inches(1.55), Inches(0.85))
apply_fill(hub, ACCENT)
hub.line.fill.background()
add_centered_lines(hub, ['MCP'], [DARK_TEXT], [18], [True])
for idx in range(3):
    y = Inches(3.55 + idx * 0.55)
    left_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(6.15), y, Inches(1.25), Inches(0.35))
    apply_fill(left_box, CALLOUT_BG)
    apply_line(left_box, ACCENT, 1.1)
    add_centered_lines(left_box, ['AI App'], [TITLE], [11], [False])
    add_connector_arrow(slide, Inches(7.40), y + Inches(0.17), Inches(8.00), Inches(4.58), color=ACCENT, width=1.0)
    right_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(10.25), y, Inches(1.25), Inches(0.35))
    apply_fill(right_box, CALLOUT_BG)
    apply_line(right_box, ACCENT, 1.1)
    add_centered_lines(right_box, ['Tool'], [TITLE], [11], [False])
    add_connector_arrow(slide, Inches(9.55), Inches(4.58), Inches(10.25), y + Inches(0.17), color=ACCENT, width=1.0)
add_callout(slide, Inches(1.4), Inches(5.80), Inches(10.4), Inches(0.90),
...
add_textbox(slide, Inches(5.4), Inches(1.65), Inches(3.0), Inches(0.3), 'Tools exposed:',
            font_size=16, color=ACCENT, bold=True, margin=0.0)
add_table(slide, Inches(5.35), Inches(2.0), Inches(7.0), Inches(2.70),
          ['Tool', 'Description'], [
              ...
              ['login / logout', 'Browser-based auth management'],
          ], [Inches(2.35), Inches(4.65)], font_size=11.2)
add_code_block(slide, Inches(0.85), Inches(4.85), Inches(5.0), Inches(1.45),
               ...
               font_size=11.5, highlight_lines={1})
...
browser = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(0.75), Inches(1.45), Inches(12.0), Inches(0.82))
apply_fill(browser, UI_BLUE)
browser.line.fill.background()
add_centered_lines(browser, ['Your Browser — React Chat UI  •  SSE stream  •  FastAPI (port 8000)'], [TITLE], [18], [True])
bypass = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(8.30), Inches(2.55), Inches(3.55), Inches(0.95))
apply_fill(bypass, API_PURPLE)
apply_line(bypass, ACCENT, 1.5)
add_centered_lines(bypass, ['server.py', 'Bypass Layer • JWT interception • Confirmation bypass'], [TITLE, TITLE], [17, 11.5], [True, False])
agent_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(4.05), Inches(3.95), Inches(5.15), Inches(0.95))
apply_fill(agent_box, PYTHON_GREEN)
agent_box.line.fill.background()
add_centered_lines(agent_box, ['LangGraph Agent', 'MemorySaver • phi4-mini via ChatOllama'], [TITLE, TITLE], [18, 12], [True, False])
transport = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(3.65), Inches(5.10), Inches(5.95), Inches(0.62))
apply_fill(transport, CALLOUT_BG)
apply_line(transport, ACCENT, 1.4)
add_centered_lines(transport, ['MCP stdio transport • langchain-mcp-adapters'], [TITLE], [14], [False])
mcp_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(3.20), Inches(5.95), Inches(6.85), Inches(0.85))
apply_fill(mcp_box, NODE_ORANGE)
mcp_box.line.fill.background()
add_centered_lines(mcp_box, ['myschedule-mcp (Node.js / TypeScript)', 'search_roles • view_role • apply_role'], [TITLE, TITLE], [17, 12], [True, False])
api_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(3.55), Inches(6.95), Inches(6.15), Inches(0.36))
apply_fill(api_box, API_PURPLE)
api_box.line.fill.background()
add_centered_lines(api_box, ['MySchedule REST API  •  Azure AD auth'], [TITLE], [14], [True])
add_connector_arrow(slide, Inches(6.75), Inches(2.27), Inches(6.75), Inches(3.95), color=ACCENT, width=2.0)
add_connector_arrow(slide, Inches(8.30), Inches(3.02), Inches(9.20), Inches(3.02), color=ACCENT, width=1.6)
add_connector_arrow(slide, Inches(6.62), Inches(4.90), Inches(6.62), Inches(5.10), color=ACCENT, width=2.0)
add_connector_arrow(slide, Inches(6.62), Inches(5.72), Inches(6.62), Inches(5.95), color=ACCENT, width=2.0)
add_connector_arrow(slide, Inches(6.62), Inches(6.80), Inches(6.62), Inches(6.95), color=ACCENT, width=2.0)
add_textbox(slide, Inches(0.95), Inches(2.70), Inches(2.6), Inches(0.75), 'Everything here is local.\nOnly the MySchedule API call leaves the laptop.',
            font_size=14, color=BODY, align=PP_ALIGN.LEFT, margin=0.0)
...
add_textbox(slide, Inches(0.75), Inches(1.50), Inches(5.6), Inches(0.35),
            "Trigger: user says 'yes', 'confirm', 'proceed'", font_size=15, color=ACCENT, bold=True, margin=0.0)
add_textbox(slide, Inches(0.75), Inches(1.92), Inches(2.2), Inches(0.3), 'Path 1 — LLM called view_role:',
            font_size=15, color=TITLE, bold=True, margin=0.0)
add_numbered_paragraphs(slide, Inches(0.9), Inches(2.22), Inches(5.5), Inches(1.55), [
    'Read MemorySaver state',
    'Find last view_role ToolMessage',
    'Extract projectKey, projectLocationKey, contact email',
    'Call apply_role directly — no LLM',
], font_size=14.5)
add_textbox(slide, Inches(0.75), Inches(3.95), Inches(2.0), Inches(0.3), 'Path 2 — Fallback:',
            font_size=15, color=TITLE, bold=True, margin=0.0)
add_numbered_paragraphs(slide, Inches(0.9), Inches(4.25), Inches(5.5), Inches(1.15), [
    'Regex-scan the last 6 AI messages for a role ID',
    'Call view_role to get real keys',
    'Call apply_role with verified data',
], font_size=14.5)
add_textbox(slide, Inches(0.75), Inches(5.55), Inches(2.8), Inches(0.3), 'Bonus — JWT interception:',
            font_size=15, color=TITLE, bold=True, margin=0.0)
add_bullets(slide, Inches(0.85), Inches(5.82), Inches(5.6), Inches(0.68), [
    'Paste Azure AD token → intercept server-side → save to token.txt → re-seed MCP auth',
], font_size=13.5)
msg_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(7.10), Inches(1.85), Inches(3.45), Inches(0.55))
apply_fill(msg_box, CALLOUT_BG)
apply_line(msg_box, ACCENT, 1.5)
add_centered_lines(msg_box, ['User says "yes"'], [TITLE], [17], [True])
diamond = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.DIAMOND, Inches(7.55), Inches(2.70), Inches(2.55), Inches(1.05))
apply_fill(diamond, YELLOW)
diamond.line.fill.background()
add_centered_lines(diamond, ['Confirmation?'], [DARK_TEXT], [15], [True])
llm_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(10.55), Inches(2.78), Inches(1.55), Inches(0.70))
apply_fill(llm_box, UI_BLUE)
llm_box.line.fill.background()
add_centered_lines(llm_box, ['LLM'], [TITLE], [15], [True])
bypass_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(7.05), Inches(4.15), Inches(3.55), Inches(1.05))
apply_fill(bypass_box, API_PURPLE)
apply_line(bypass_box, ACCENT, 1.5)
add_centered_lines(bypass_box, ['Bypass path', 'MemorySaver / regex → view_role → apply_role'], [TITLE, TITLE], [17, 11], [True, False])
done_box = slide.shapes.add_shape(MSO_AUTO_SHAPE_TYPE.ROUNDED_RECTANGLE, Inches(7.60), Inches(5.65), Inches(2.45), Inches(0.55))
apply_fill(done_box, GREEN)
done_box.line.fill.background()
add_centered_lines(done_box, ['Application submitted'], [TITLE], [15], [True])
add_connector_arrow(slide, Inches(8.82), Inches(2.40), Inches(8.82), Inches(2.70), color=ACCENT, width=1.8)
add_connector_arrow(slide, Inches(10.10), Inches(3.22), Inches(10.55), Inches(3.13), color=GREY, width=1.4)
add_textbox(slide, Inches(10.22), Inches(2.55), Inches(0.35), Inches(0.18), 'NO', font_size=10.5, color=GREY, bold=True, margin=0.0)
add_connector_arrow(slide, Inches(8.82), Inches(3.75), Inches(8.82), Inches(4.15), color=ACCENT, width=1.8)
add_textbox(slide, Inches(9.02), Inches(3.86), Inches(0.35), Inches(0.18), 'YES', font_size=10.5, color=ACCENT, bold=True, margin=0.0)
add_connector_arrow(slide, Inches(8.82), Inches(5.20), Inches(8.82), Inches(5.65), color=ACCENT, width=1.8)
...
add_bullets(slide, Inches(0.95), Inches(4.25), Inches(11.0), Inches(1.90), [
...
add_bullets(slide, Inches(0.75), Inches(1.55), Inches(5.75), Inches(3.20), [
    ...
    'Critical bug fix: React setState is async — capture thread_id locally before state updates',
], font_size=15)
add_code_block(slide, Inches(6.65), Inches(1.95), Inches(5.1), Inches(2.80),
...
add_textbox(slide, Inches(0.85), Inches(3.45), Inches(3.3), Inches(0.3), 'Why CPU-only for the model:',
            font_size=16, color=ACCENT, bold=True, margin=0.0)
add_bullets(slide, Inches(0.9), Inches(3.8), Inches(5.25), Inches(1.95), [
    'Intel Arc 140V uses Vulkan backend (not CUDA)',
    'phi4-mini + 4096-token KV cache ≈ 2.6 GB exceeds Vulkan shared GPU memory limit',
...
add_table(slide, Inches(0.7), Inches(1.55), Inches(11.95), Inches(3.1),
          ...
          ], [Inches(3.4), Inches(8.55)], font_size=11.5)
...
output_path = r'c:\dev\local-llm-phi4\slide-deck-gpt4.pptx'
